(function () {
      ...
      timers.set(HOME_INTRO_TIMING.headingLeadIn, function () {
        ...
        logo
          .playIntro(function () {
            ...
            return settleHeroLift().then(function () {
              timers.set(HOME_INTRO_TIMING.restRevealLag, resolveIntro);
            });
          })
  ...
  function createHeroLogoController(options) {
    ...
    function composeCells(settled, active, options) {
      ...
      if (renderOptions.settledColor) {
        for (const position of Object.keys(cells)) {
          if (cells[position] !== "flash") {
            cells[position] = renderOptions.settledColor;
          }
        }
      }
    ...
    async function animatePiece(settled, piece, duration, run) {
      ...
      const frames = Math.max(Math.round(scaledDuration / frameMs), 7);
    ...
    async function playLogoSequence(onBeforeClear, run) {
      let settled = {};
      ...
      const finalLogoColor = settledLogoColor();
      for (let i = 0; i < LOGO_TIMING.clearFlashCount; i++) {
        if (
          !(await holdLogo(
            settled,
            LOGO_TIMING.clearFlashStep,
            { flashClearRow: i % 2 === 0, settledColor: i % 2 === 0 ? finalLogoColor : null },
            run,
          ))
        ) {
          return false;
      ...
      for (const [position] of Object.entries(settled)) {
        if (parseCellKey(position).y !== CLEAR_ROW) {
          floating[position] = finalLogoColor;
        }
      }
    ...
    async function dropLogoDown() {
      ...
      if (!(await holdLogo({}, 56, { whiteLogo: true }, run))) {
    ...
    function playIntro(onBeforeClear) {
      ...
      const run = createLogoRun();
      activateTallCanvas();
      return playLogoSequence(onBeforeClear, run).finally(function () {
